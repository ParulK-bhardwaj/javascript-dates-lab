...
const ageInSec = ageInMs / 1000
console.log(ageInSec, '<- Age in sec')

const ageInMin = ageInSec / 60 
console.log(ageInMin, '<- Age in min')

const ageInHrs = ageInMin / 60 
console.log(ageInHrs, '<- Age in hrs')

const ageInDays = ageInHrs / 24
console.log(ageInDays, '<- Age in days')

const ageInYrs = ageInDays / 365.25
console.log(ageInYrs, '<- Age in years')
...
console.log(`The day I was born: ${days[myBday.getDay()]}`)
...
function consecutiveDates(date, repeat, offset) {
  const consecutiveDateList = [];
  
  for (let i = 0; i < repeat; i++) {
      const newDate = new Date(date);
      newDate.setDate(date.getDate() + (i * offset));
      consecutiveDateList.push(newDate);
  }
  
  return consecutiveDateList;
}
...
function consecutiveDatesYearly(date, repeat, offset, unit = 'year') {
  const consecutiveDateList = [];
...
console.log(consecutiveDatesYearly(new Date(2019, 0, 1), 4, 3))
// 1. 1/1/2019
// 2. 1/1/2020
// 3. 1/1/2021
...
console.log(orderDates([today, dueDate, startDate, myBday, newYear]))
